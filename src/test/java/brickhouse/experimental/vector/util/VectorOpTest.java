...
public class VectorOpTest {
  ...
  public void testScalarOP() throws Exception {
    ...
    assert "{a=3.0, b=4.0}".equals(VectorOp.evaluate(vec, 2.0, "+").toString());
    assert "{a=-1.0}".equals(VectorOp.evaluate(vec, 2.0, "-").toString());
    assert "{a=2.0, b=4.0}".equals(VectorOp.evaluate(vec, 2.0, "*").toString());
    assert "{a=0.5, b=1.0}".equals(VectorOp.evaluate(vec, 2.0, "/").toString());
  }
  ...
  public void testVectorOP() throws Exception {
    ...
    // Note: If vec1 has elements that are not present in vec2, they get a value of Infinity i.e. division by 0 error.
    // Workaround:     udf.evaluate(udf.evaluate(vec1, vec2, "select_by_key"), vec1, "/")
    vec2.put("d", 4.0);
    assert "{a=0.5, b=0.5}".equals(udf.evaluate(vec1, vec2, "/").toString());

    vec1.put("c", 2.0);
    assert "{a=0.5, b=0.5, c=Infinity}".equals(udf.evaluate(vec1, vec2, "/").toString());
    assert "{a=0.5, b=0.5}".equals(udf.evaluate(udf.evaluate(vec1, vec2, "select_by_key"), vec2, "/").toString());
  }
  ...
  public void testFilterOP() throws Exception {
    ...
    assert "{b=2.0, c=3.0}".equals(VectorOp.evaluate(vec1, 1.5, ">").toString());
    assert "{a=1.0}".equals(VectorOp.evaluate(vec1, 1.5, "<").toString());

    assert "{a=1.0, c=3.0}".equals(VectorOp.evaluate(vec1, vec2, ">").toString());
    assert "{b=2.0}".equals(VectorOp.evaluate(vec1, vec2, "<").toString());
    ...
    String result = VectorOp.evaluate(vec3, vec4, "select_by_key").toString();
    assert "{a=1.0, b=2.0}".equals(result) ||
        "{b=2.0, a=1.0}".equals(result);
    assert "{c=3.0}".equals(VectorOp.evaluate(vec3, vec4, "filter_by_key").toString());
    result = VectorOp.evaluate(vec4, vec5, "filter_by_key_with_defaults").toString();
    ...
    result = VectorOp.evaluate(vec1, vec2, "filter_by_prefix").toString();
    assert "{cde=3.0}".equals(result);

    vec1.put("hello", 1.0);
    vec2.put("el", 1.0);

    vec1.put("cards.twitter.com", 1.0);
    vec1.put("mobile.twitter.co", 1.0);
    vec2.put("twitter", 1.0);

    result = VectorOp.evaluate(vec1, vec2, "filter_if_contains").toString();
    assert "{cde=3.0}".equals(result);

    result = VectorOp.evaluate(vec1, vec2, "filter_not_if_contains").toString();
    assert "{hello=1.0, b=2.0, aa=1.0, cards.twitter.com=1.0, mobile.twitter.co=1.0}".equals(result);

    vec1.clear();
    vec2.clear();
    vec1.put("mobile.twitter.com", 1.0);
    vec2.put("hello", 1.0);

    result = VectorOp.evaluate(vec1, vec2, "filter_if_contains").toString();
    assert "{mobile.twitter.com=1.0}".equals(result);
  ...
  public void testKeyStrOpOP() throws Exception {
    ...
    assert "{1:a=1.0, 1:b=2.0, 1:c=3.0}".equals(VectorOp.evaluate(vec1, "1:", "add_key_prefix").toString());
    assert "{a:2=1.0, b:2=2.0, c:2=3.0}".equals(VectorOp.evaluate(vec1, ":2", "add_key_suffix").toString());
  }
  ...
  public void testLowerKey() throws Exception {
    ...
    assert "{aa=4.0, bb=6.0, cc=5.0}".equals(VectorOp.evaluate(vec1, "lower_key").toString());
  }
  ...
  public void normalize() throws Exception {
    ...
    assert "{a=0.1, b=0.2, c=0.3, d=0.4}".equals(VectorOp.evaluate(vec1, "normalize").toString());
  }

  @Test
  public void vectorNormalize() throws Exception {
    HashMap<String, Double> vec1 = new HashMap<String, Double>();
    vec1.put("a", 1.0);
    vec1.put("b", 1.0);
    vec1.put("c", 1.0);
    ...
    assert "{a=0.5, b=0.5, c=0.5, d=0.5}".equals(VectorOp.evaluate(vec1, "vector_normalize").toString());
  }
  ...
  public void precisionRound() throws Exception {
    ...
    Map<String, Double> val = VectorOp.evaluate(vec1, 0.02 ,"round");
  ...
  public void precisionCeil() throws Exception {
    ...
    Map<String, Double> val = VectorOp.evaluate(vec1, 0.02 ,"ceil");
    assert almostEqual(val, expectedRound, 0.0001);
  }
  ...
  public void sanitizeIndexRankTest() throws Exception {
    ...
    val = VectorOp.evaluate(vec, "sanitize");
  ...
  public void convolutionCircularTest() throws Exception {
    ...
    val = VectorOp.evaluate(vec1, vec2, "convolution_circular");
  ...
  public void convolutionTest() throws Exception {
    ...
    val = VectorOp.evaluate(vec1, vec2, "convolution");
  ...
  public void testCap() throws Exception {
    ...
    Map<String, Double> result = VectorOp.evaluate(vec, -0.5, "cap_lower");
    assert result.toString().equals("{a=-0.5, b=0.0, c=1.0}") : "Got " + result ;
    result = VectorOp.evaluate(vec, 0.5, "cap_upper");
    assert result.toString().equals("{a=-1.0, b=0.0, c=0.5}") : "Got " + result ;

    result = VectorOp.evaluate(null, 0.5, "cap_lower");
    assert result == null;
  }
}
